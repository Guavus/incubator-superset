...
class SqlEditorLeftBar extends React.PureComponent {
  ...
  getTableNamesBySubStr(input) {
    ...
    const url = `${baseUrl}/superset/tables/${this.props.queryEditor.dbId}/` +
  ...
  fetchTables(dbId, schema, substr) {
    ...
    if (dbId && schema) {
      ...
      const url = `${baseUrl}/superset/tables/${dbId}/${schema}/${substr}/`;
      $.get(url).done((data) => {
  ...
  fetchSchemas(dbId) {
    ...
    if (actualDbId) {
      ...
      const url = `${baseUrl}/superset/schemas/${actualDbId}/`;
      $.get(url).done((data) => {

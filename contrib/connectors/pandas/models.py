...
class PandasDatasource(Model, BaseDatasource):
    ...
    def process_dataframe(
        ...
        # print('filtered metrics    {}'.format(filtered_metrics))
        for m in metrics + filtered_metrics:
            try:
                # print('loop metrics    {}'.format(m))
                # print('dict metrics    {}'.format(metrics_dict[m]))
                if(isinstance(m, dict)):
                    print("frozen set {}".format(m))
                    # for key in m:
                    #     if(isinstance(key, dict)):
                    #         for innerkey in key:
                    #             metric = metrics_dict[str(innerkey)]
                    #     else:
                    #         metric = metrics_dict[str(key)]
                else:
                    metric = metrics_dict[str(m)]
                # metric = self.freeze(m, metrics_dict)
            except KeyError:
                raise Exception(_("Metric '{}' is not valid".format(m)))
            print("MM {}".format(m))
            print("metrics {}".format(metric))
            print("metrics_exprs {}".format(metrics_exprs))
            if(isinstance(m, dict)):
        ...
        if filter:
            ...
            print('query is: {}'.format(filter_str))
            print('query before: {}'.format(df))
